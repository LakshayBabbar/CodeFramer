...
export function SpotLightItem({ children, className }: SpotlightItemProps) {
    ...
    const handleMouemove = ({ currentTarget, clientX, clientY }: { currentTarget: EventTarget & HTMLDivElement; clientX: number; clientY: number }) => {
    ...
    return (
        <div
            ...
            className={cn(
                className,
                ' relative rounded-lg justify-center items-center p-[2px] dark:bg-[#ffffff15] bg-[#0000000c] overflow-hidden'
            )}
        >
            {isHovered && (
                <div
              ...
              ${theme === "dark" ? "rgba(255, 255, 255, 0.137)" : "rgba(0, 0, 0, 0.090)"},
              transparent 80%
            )`,
                    }}
                />
            ...
            {ProximitySpotlight && (
                <div
                    ...
                    style={{
                        background: `radial-gradient(circle at ${mousePosition.x}px ${mousePosition.y}px, ${theme === "dark" ? "#ffffff6e" : "#0006"} 0%,transparent 20%,transparent) fixed`,
                    }}
                ></div>
            )}
            {children}
